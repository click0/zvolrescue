...
| **Debian stable** | most common Linux host with `zfs-dkms`; the cheapest real kernel: loop devices in a VM | zfs-dkms 2.1.11 (12 bookworm), 2.3 (13 trixie: `raidz_expansion`) | `apt install zfsutils-linux zfs-dkms`; loop devices; scripted — see [Running on Debian with zfs-dkms and loop devices](#running-on-debian-with-zfs-dkms-and-loop-devices). **Run**: Debian 12 as a QEMU/KVM guest in `.github/workflows/realworld.yml`, on demand; the row is in Results |
| **Ubuntu LTS** | ships ZFS in-kernel, common on hosts | zfs 2.2/2.3 | the CI cross-check runs here already (userland only). **Run**: the same script on the runner's own kernel and in-tree module, the first job of `realworld.yml`; the row is in Results |
