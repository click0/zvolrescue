...
* Reading failing media. Physical and logical recovery are separate
  trades (N-10): a disk with defects is imaged first, by a tool made for
  that — `ddrescue` or HDDSuperClone for the simple cases, PC-3000 with
  Data Extractor where the defect has to be understood first — and this
  tool works on the image and its map (F-72). It reads no SMART, patches
  no firmware, knows nothing of zones or heads, and stops at the first
  read a device refuses (F-33). A separate imaging tool is a possible
  later project, outside this repository (§10).
...
| F-14 | S | **Read dataset properties (ZAP), including user properties.** `list --properties` reports what each dataset has *set*: the ZAP of its DSL directory, or a snapshot's own. Only that is on disk — a property nobody set is inherited from an ancestor or is the pool's default, and neither is written down anywhere, so the answer is what the dataset holds and the ancestors are in the same listing. Values keep their numbers unless this build can show its working for a name: `checksum` and `dedup` carry the block pointer's own `zio_checksum` enumeration, and `compression` its `zio_compress` code with a `zstd` level above it. Everything else — `copies`, `recordsize`, every on/off flag — prints the number the pool holds rather than a name that might be the wrong one. User properties are text and are reported as stored, names and suffixes unchanged. |
| F-15 | S ◇ | List pending deletions (`dp_free_bpobj`, deadlists) so the user can judge whether destroyed data is still on disk. |
| F-69 | C | **A pool a vdev was removed from.** `device_removal` copies a top-level vdev's blocks elsewhere and leaves an `indirect` vdev holding a mapping from the old DVAs to the new ones. The pointers are never rewritten — that would mean walking every pointer in the pool — so they go on naming a vdev that is gone. Read that mapping (`vdev_indirect_mapping`) and translate through it: a range the removal copied in pieces is several entries and is joined back together, and a piece that landed on a vdev itself removed later is followed through that one too. The mapping comes from the pool's configuration object in the MOS being read, never from the labels: no label describes a removed vdev, because removal is what took its members away. Reading it there is also what keeps an older transaction group honest — asked for one from before the removal, the configuration of *that* txg has no indirect vdev in it and nothing is translated. A removed vdev is still counted in `vdev_children` and still has no member to find; from the labels alone it is a missing vdev. `scan` asks the pool's configuration object which undescribed vdevs were removed — only when one is undescribed and `device_removal` is active, so a healthy pool never opens its MOS for this — and reports them as removed, not missing, with the pool readable. When the MOS cannot be reached it says so and leaves them missing, which they may be. A range the mapping does not cover is refused by name, per block, and the rest of the pool reads normally. |
...
| F-28 | S | Dedup: resolve DDT-referenced blocks transparently (they are just blkptrs) — no special handling required beyond checksum handling. Checked on a fixture volume whose data pointers carry the dedup bit and sha256 checksums, with `dedup=sha256,verify` set on it (`mkfixture … dedup`): `list --properties` names the property, and the image is the plain volume's, byte for byte. |
...
| F-32 | S | Resume interrupted extractions (block-level progress file). A SIGINT or SIGTERM ends `dump` at the next block boundary with the state written — the block it was about to read — and exit `6`; a bulk run (`-r`) does not go on to the next volume. A second signal is the default action. The state is also checkpointed every 5 seconds against a kill that no handler sees. The companions do the same: `zvolcarve dump` shares this state file and `--resume`, `zvolcarve scan` stops at the next chunk with its own state, `zvolfiles extract` at the next entry or block with the manifest saying what was written (COMPANIONS §1.2). |
| F-33 | M | **Partial-result policy, and where a bad sector may cost only its sector.** On unreadable blocks write zeros (default) or abort (`--strict`), always logging the range and the reason. What happens at a refused read depends on what refused it (N-10). **On an image with its map (F-72)** the map refuses the range before anything is read, and the block is kept as far as the imager read it: on an uncompressed, unencrypted block on a disk or mirror every sector the map allows is kept, only the refused ones are zeros, each logged as its own range with the note that what was kept is unverified, since the checksum covers the whole block; a compressed or encrypted block, or one under raidz/dRAID, cannot be kept in part and is zeroed whole, with the reason. **On a device** the first read the medium refuses stops the run: the incident — byte offset and LBA, length, `errno`, time, which member, which volume and block — goes to stderr and into the evidence record, the exit code is `7`, and the message says what to do next: image the disk with a tool made for failing media, with a map, and continue on the image with `--map` and `--resume`. Nothing is read twice: no retry, no re-read in pieces, no sector-by-sector pass — those cost a failing disk what it has left and gain nothing a user-space program can control. `--device-may-fail` is the one way past the first incident, for the operator who has weighed it and needs one small object off a disk that cannot be imaged: the refused range is skipped once, zeroed, logged, and after 8 incidents the run stops anyway. **On an image without a map** a read error is the operator's filesystem, not the medium: zeros, the reason, exit `4`. |
| F-72 | S | **The imager's map.** An image of a failing disk is made with GNU `ddrescue`, and the mapfile beside it is the only record of which bytes the disk actually gave; where it could not read, the image holds zeros that look like data, and a block crossing them fails its checksum for no reason a reader can name. `--map MEMBER=MAPFILE` reads that map: every unfinished range (`-`, `/`, `*`, `?`) is refused before it is read, exactly as a disk refuses a bad sector, so the block takes the F-33 path — kept as far as the imager read it, zeros for the rest, the imager named as the reason. `scan` reports the map: bytes never read, by status, and which of the four labels the unread ranges touch. The mapfile is an input in the evidence record, hashed like the members. This is the one path on which a bad sector costs only its sector: the map decides, and the medium is never asked twice. |
...
| F-71 | S | **The disk's own names.** A member is often given to ZFS by a name the disk itself carries: a GPT partition label (`gpart -l`, `/dev/gpt/NAME`; Linux `by-partlabel`), the partition's unique GUID (`/dev/gptid/…`; Linux `by-partuuid`), or a `glabel` written into the provider's last sector (`/dev/label/NAME`), and the pool's labels record that name as the member's `path`. When the member's own labels are gone that name is what still ties it to the leaf its siblings describe (F-62). `scan` reads the names off every device — GPT name and GUID, GEOM metadata at the end of the device and of each partition — lists them, and where a vacant leaf's `path` is one of them says so and spells out the `--assume-member` that binds it; the read then confirms or refuses the binding as F-62 does, never the name alone. A name that merely contains a pool's name is reported as a hint and binds nothing. GEOM metadata also records the size of the provider it sits on, and ZFS saw one sector less, which is where its rear labels are; `geli` is named for what it is. |
| F-63 | C ◇ | **Pointer self-consistency, fully automatic** (the worst case, all rings gone and no hints): scan for structures recognisable without a base (dnode arrays — type ≤ 54, `indblkshift` 9..17, small `nlevels`/`nblkptr`, 512-byte period; indirect blocks; objset headers), collect their block pointers, and confirm a candidate base `B` by checking that the block at `B + 4 MiB + offset` has the pointer's checksum. Candidates step by `1 << ashift` (`ashift` itself follows from the smallest DVA offset step and `asize` granularity) inside the alignment window. A wrong base is expected to pass no check — but it can pass a few, because a shifted base lines a pointer up with a *different* block of identical content, and a sparse member is mostly zeros. So what is reported is how many independent pointers agreed out of how many were read, not a yes or a no: on a `ztest` pool with every label gone the true base agreed with 46 of 64 probes and every other candidate with exactly 1. Gang headers (verifier `[vdev, offset, birth]`) and ZIL chains (`zc_next_blk` vs. the physical position of the next block) are extra anchors. A block device is not scanned for this without `--surface-scan-on-device` (N-10). |
| F-64 | S ◇ | **Root without uberblocks**: once the base is known and no uberblock survives, find the MOS by scanning for `objset_phys` candidates of type META, rank them by the highest `birth` in their pointers and by how complete a MOS walk they yield, then continue through the DSL as usual. This is `zvolcarve` territory (F-41/F-42), not the atomic binary's. A block device is not scanned for this without `--surface-scan-on-device` (N-10). |

**Implemented so far.** F-60's cheapest anchor is in: given an image of a
whole disk, the GPT (primary or backup, 512- or 4096-byte sectors) or MBR
is read, its ZFS partition types are marked, and every partition start is
a base candidate — accepted only where a label checksum verifies, since a
rewritten table is one of the ways a pool stops importing. The
partition's *length* is used as well: the rear label pair sits against
the vdev's own size, so a member read to the end of the disk rather than
the end of its partition finds two of its four labels. That is F-06 as
far as the atomic binary needs it. F-61 is in the tool: `scan` searches a member for
...
| N-03 | **Scales to multi-TB pools.** Streaming extraction with bounded memory (target: ≤ 512 MiB RSS regardless of pool size, excluding optional caches). Measured on every push: `dump` reports its peak resident set (`peak_rss_kib`, Linux) and `tests/bench.sh` fails a run over the limit on 512 MiB dense volumes, mirror and raidz2, every compression the fixture encodes. |
...
| N-05 | **Reproducible.** Same inputs + same arguments → byte-identical outputs and reports (timestamps only in designated fields: the evidence record's `ts`, and `seconds` and `peak_rss_kib` on a `dump`). Checked on every push: the same volume dumped, scanned and listed twice, images and documents compared. |
| N-06 | **Portable.** FreeBSD **15.x is the first-class target**: CI builds and runs the suite there on every push, and the published FreeBSD binary is built there. **14.x is kept on a best-effort footing** — a CI job builds and tests it, and a failure is reported without blocking a release; `stable/14` is supported by the project until November 2028, so it stays reachable. 13.x is dropped: it left support in April 2026. Linux (glibc/musl) builds and passes the test suite; macOS is best-effort. |
| N-06a | **A FreeBSD binary is a major-branch binary.** FreeBSD keeps its ABI within a major branch and changes it between majors, so the published `amd64-freebsd` file is built on 15.x and is the 15.x file. On 14.x, build from source — `pkg install rust && cargo build --release` is the whole procedure, and the test suite passes there. Shipping a second 14.x-built binary is the alternative if downloads matter more than one file per platform. |
| N-07 | **Auditable.** Small dependency footprint, no network access, deterministic builds, signed release tarballs. |
| N-08 | **Performance target.** Sequential zvol extraction ≥ 70 % of raw device read throughput on a mirror; ≥ 40 % on RAIDZ2 (single thread), with an optional worker pool for parallel checksum/decompression. The ratio is to a *disk's* read rate and is settled on real disks (REALWORLD-TESTS E1, E2); `tests/bench.sh` measures the absolute rate on every push against a plain copy through the same page cache, prints it, and fails only under a floor. |
| N-09 | **Localisation.** UI strings are English; documentation is maintained in English and Ukrainian. |
| N-10 | **The medium policy: this tool reads healthy media.** An input is an image (a regular file) or a device (a block or character device; F-68 tells them apart). Metadata is parsed and volumes are extracted from images, clones, or devices that read cleanly. A device is read sequentially and in large blocks where the work allows, no address is ever read twice, a device that refused a read and stopped the run is not read again for the rest of it — the io layer refuses every later read of that device, whatever asks, before touching it; other devices and every image go on being read — and the surfaces are never scanned: the zero-point search (F-61), the root search (F-64), pointer self-consistency (F-63) and carving (COMPANIONS C-01) refuse a device with exit `5` unless `--surface-scan-on-device` is given, and the help text says why that flag is dangerous on a disk with defects. The first read a device refuses stops the run with exit `7` (F-33). Physical and logical recovery are separate trades: a failing disk is imaged first, by a tool built for that — one that knows the defect's kind, the zones and the heads, when to stop — and this tool works on the image and its map (F-72). It reads no SMART, patches no firmware, builds no zone map and has no read strategy, and says so rather than pretend. |
...
aborted at the first one — `5` refused (would write to evidence, or would
scan a device), `6` interrupted by a signal, the state written (`--resume` continues), `7` the medium refused
a read: a device, not an image, returned an I/O error and the run stopped
there (F-33, N-10).
