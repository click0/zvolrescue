...
скрипта ганяє приклад `walk-objects` по всіх файлах каталогу пулу: кожен
блок кожного об'єкта кожного dataset-у читається з перевіркою контрольної
суми (усі алгоритми, із сіллю пулу), розпаковується, а кількість об'єктів
dataset-у порівнюється з fill його objset-вказівника. Шифровані dataset-и
обходяться настільки, наскільки сягає відкритий текст (objset, непрямі
блоки); блоки-шифротекст рахуються, а не вважаються помилкою. Саме це
зловило помилки zstd, sha512 і контрольних сум шифрованих блоків, які
синтетичні фікстури не могли виявити. Запустити самому:
