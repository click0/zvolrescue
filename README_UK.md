...
**Мова:** Rust | **Ліцензія:** BSD 3-Clause | **Статус:** v0.9.7 (див. [CHANGELOG.uk.md](CHANGELOG.uk.md)) — `scan`, `list` і `dump` працюють на stripe, mirror, RAIDZ1/2/3 і dRAID пулах (відновлення за парністю відсутніх і тихо зіпсованих колонок, розподілені spare), gang-блоках, з checksum-ами fletcher/sha256/sha512/blake3/skein/edonr (усі алгоритми OpenZFS), стисненням lz4/zstd/gzip/lzjb/zle, `--resume` і масовим `-r`; шифровані dataset-и: `list` показує suite і формат ключа, `dump --key` (raw/hex/passphrase/prompt) розшифровує (етап 3; без ZIL). Пули, у яких зникли мітки, теж читаються: нульова точка з уцілілого уберблока, член, прив'язаний до листа, який називають сусіди — або до листа, який називає його власний диск: GPT-мітка, `gptid` чи `glabel`, — розкладка, задана руками, і підтверджена розкладка, виписана назад як мітка. Кожен `ashift` від 9 до 13 перевіряється, 9, 12 і 13 — проти `zdb`. На кожному пуші перевіряються граф залежностей і таблиця імпортів кожного бінарника, кожен парсер fuzz-иться, юніт-тести дискового формату виконуються під Miri; файли релізу підписані без ключа. Цей інструмент читає справні носії: перше відмовлене читання на пристрої зупиняє роботу (код 7), ніщо не читається двічі, а диск із дефектами — спершу для імаджера (ТЗ N-10). Пам'ять і пропускна здатність вимірюються на кожному пуші на щільних томах по 512 МіБ, на дзеркалі й raidz2, а власний декодер zstd — проти libzstd на тих самих блоках. Звірено з userland OpenZFS (`ztest`/`zdb`) у CI; на справжньому пулі, імпортованому ядром, ще не запускалось. Див. [ТЗ](docs/SPEC.uk.md).
...
## Перш ніж почати: диск чи його образ?

`zvolrescue` читає **справні носії**: файли-образи, клони або диски, що
читаються чисто (ТЗ N-10). Це не імаджер, і з несправним диском він не
працює. Фізичне і логічне відновлення — різні ремесла: диск із дефектами
— bad-сектори, деградуюча головка, SSD із дірками в трансляції — спершу
знімають в образ інструментом, збудованим для цього, який знає рід дефекту
і коли зупинитись (`ddrescue` з mapfile для простого випадку; PC-3000 із
Data Extractor або HDDSuperClone там, де дефект треба спершу зрозуміти).
Потім цей інструмент працює з образом і його картою (`--map
ОБРАЗ=MAPFILE`, ТЗ F-72).

На пристрої інструмент читає послідовно, жодної адреси не просить двічі й
ніколи не шукає по поверхні. Перше читання, яке пристрій відмовив, зупиняє
роботу з кодом виходу 7: інцидент — пристрій, зміщення в байтах і LBA,
довжина, помилка, час — на stderr і в записі доказів, а повідомлення
каже, що робити далі. Нічого не повторюється. `--device-may-fail` —
єдиний шлях повз цю зупинку, для оператора, який зважив і потребує один
невеликий об'єкт із диска, образ якого зняти неможливо: кожен відмовлений
діапазон пропускається один раз, а після восьми інцидентів робота все
одно зупиняється.
...
`zvolcarve` і `zvolfiles`, а також `SHA256SUMS`. Файли для Linux — збірки
musl, працюють на будь-якому дистрибутиві; файл для FreeBSD має влінковану
власну libc FreeBSD і працює на будь-якій системі 15.x, включно з mfsBSD 15
(на 14.x збирайте з джерел: `pkg install rust && cargo build --release`).
Один виняток: файли `amd64-freebsd` версій v0.9.7 і раніших злінковані
динамічно з базовими бібліотеками FreeBSD 15, які є в кожній системі 15.x;
починаючи з наступного тега release-workflow робить їх статичними і
відмовляється випускати реліз із динамічно злінкованим файлом. А поки що
статичну FreeBSD-збірку будь-якого коміту чи тега можна отримати на
вимогу: Actions → «FreeBSD static binaries» → Run workflow; артефакт
містить п'ять програм, кожну перевірено через `file`, і їхній
`SHA256SUMS`. Скопіюйте бінарник на рятувальний носій і запускайте;
встановлювати нічого не треба. Перевірка:
...
FreeBSD 15.x — ціль першого класу; 14.x — за остаточним принципом (вона
збирається й проходить тести, але невдача там не тримає релізу, а
опублікований бінарник для FreeBSD — це бінарник для 15.x; на 14.x —
`pkg install rust && cargo build --release`). 13.x вийшла з підтримки і
прибрана. Linux — підтримувана платформа
