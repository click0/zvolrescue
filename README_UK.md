...
* **Член зовсім без міток** — конфігурація сусідів усе одно називає лист, яким він мусить бути; `--assume-member PATH[=GUID]` ставить його туди, а кожен прочитаний через нього блок перевіряється контрольною сумою, тож хибне твердження відхиляється, а не приймається на віру.
